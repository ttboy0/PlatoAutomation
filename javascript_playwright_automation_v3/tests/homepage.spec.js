...
const PAGE_URL = "https://platotech.com/";
const DATA_FILE = path.join(__dirname, "..", "data", "homepage_data.csv");
...
test.describe("homepage page tests", () => {
    ...
    test.beforeAll(async ({ browser }) => {
        logger.info("Running beforeAll hook for homepage");
        pageElementsData = await loadCsvData(DATA_FILE);
        if (!pageElementsData || pageElementsData.length === 0) {
            logger.warn("No data loaded for homepage, tests in this file might be skipped or fail.");
        ...
        if (!navigationSuccess) {
            logger.error("Failed to navigate in beforeAll for homepage, subsequent tests may fail.");
    ...
    test.afterAll(async () => {
        logger.info("Running afterAll hook for homepage");
        if (sharedPage) {
    ...
    if (!fs.existsSync(DATA_FILE) || fs.readFileSync(DATA_FILE, "utf-8").trim() === "") {
        test("data_file_empty_or_missing", async() => {
            logger.warn("Data file is empty or missing, skipping actual tests for homepage.");
