...
const PAGE_URL = "https://platotech.com/about/";
const DATA_FILE = path.join(__dirname, "..", "data", "about_data.csv");
...
test.describe("about page tests", () => {
    ...
    test.beforeAll(async ({ browser }) => {
        logger.info("Running beforeAll hook for about");
        pageElementsData = await loadCsvData(DATA_FILE);
        if (!pageElementsData || pageElementsData.length === 0) {
            logger.warn("No data loaded for about, tests in this file might be skipped or fail.");
        ...
        if (!navigationSuccess) {
            logger.error("Failed to navigate in beforeAll for about, subsequent tests may fail.");
    ...
    test.afterAll(async () => {
        logger.info("Running afterAll hook for about");
        if (sharedPage) {
    ...
    if (!fs.existsSync(DATA_FILE) || fs.readFileSync(DATA_FILE, "utf-8").trim() === "") {
        test("data_file_empty_or_missing", async() => {
            logger.warn("Data file is empty or missing, skipping actual tests for about.");
